...
import { StartAttempt, GetAttempt, SaveAnswer, SubmitAttempt, LogViolation } from "../../api/attempt";
...
export default function StudentQuizTaking() {
  ...
  useEffect(() => {
    ...
    const handleVisibilityChange = () => {
      ...
      if (document.hidden) {
        ...
        LogViolation(attempt.id, "visible").catch(console.error);
      }
    };
    ...
    const handleFocus = () => {
      LogViolation(attempt.id, "focus").catch(console.error);
    };
    ...
    try {
      ...
      if (attemptRes.data) {
        ...
        if (attemptRes.data.status === 'submitted') {
          ...
          navigate(`/dashboard/s/${user.id}/room/${quizRes.data.room_id}/quiz/${quizId}/score`);
          return;
        }
  ...
  const handleViolation = async (eventType) => {
    try {
      ...
      if (response.data) {
        ...
        if (remaining > 0) {
          toast.error(`Warning: Tab switch detected! ${remaining} warning${remaining !== 1 ? 's' : ''} remaining`, {
            duration: 5000,
            icon: "⚠️",
          });
        } else {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      navigate(`/dashboard/s/${user.id}/room/${quiz.room_id}/quiz/${quizId}/score`);
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
      ...
      {showWarning && (
        <div className={`fixed top-0 left-0 right-0 z-50 p-4 ${isPenalized ? 'bg-red-500' : 'bg-amber-500'} text-white animate-in slide-in-from-top duration-300`}>
          <div className="max-w-5xl mx-auto flex items-center gap-3">
            <PiWarningDuotone size={24} />
            <div className="flex-1">
              <p className="font-semibold">
                {isPenalized ? 'Penalty Applied!' : 'Warning: Tab Switch Detected'}
              </p>
              <p className="text-sm">
                {isPenalized 
        ...
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <div className={`flex items-center gap-2 px-4 py-2 rounded-lg ${
              isPenalized ? 'bg-red-50 text-red-700' : 'bg-amber-50 text-amber-700'
            }`}>
