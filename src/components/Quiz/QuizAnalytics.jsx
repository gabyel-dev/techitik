...
export default function QuizAnalytics() {
  ...
  const getQuestionSuccessOption = () => {
    ...
    const questionLabels = sortedQuestions.map((q, i) => 
      q.question_text.length > 30 ? `Q${i + 1}: ${q.question_text.substring(0, 30)}...` : `Q${i + 1}: ${q.question_text}`
    );
  ...
  const getStatsSummary = () => {
    ...
    const scores = analytics.submissions.map(s => s.final_score).sort((a, b) => a - b);
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-6">
          ...
          <p className="text-slate-500 mt-1">
            {analytics?.submissions?.length || 0} submission{analytics?.submissions?.length !== 1 ? 's' : ''}
          </p>
        </div>
        ...
        <div className="mt-6 bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="p-6 border-b border-slate-200">
            <h2 className="text-xl font-bold text-slate-900">Question Performance Details</h2>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  <th className="px-6 py-4 text-left text-xs font-semibold text-slate-600 uppercase">Question</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold text-slate-600 uppercase">Points</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold text-slate-600 uppercase">Correct</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold text-slate-600 uppercase">Total</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold text-slate-600 uppercase">Success Rate</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {analytics?.questionStats?.map((q, index) => {
                  const successRate = q.total > 0 ? ((q.correct / q.total) * 100).toFixed(1) : 0;
                  return (
                    <tr key={index} className="hover:bg-slate-50">
                      ...
                      <td className="px-6 py-4">
                        <span className="text-emerald-700 font-semibold">{q.correct}</span>
                      </td>
                      <td className="px-6 py-4">
                          ...
                          <div className="flex-1 bg-slate-200 rounded-full h-2 max-w-[200px]">
                            <div
                              className={`h-2 rounded-full ${
                                successRate >= 70 ? 'bg-emerald-500' : successRate >= 50 ? 'bg-amber-500' : 'bg-red-500'
                              }`}
                              style={{ width: `${successRate}%` }}
                            />
                          </div>
                          <span className="font-semibold text-slate-900 min-w-[50px]">{successRate}%</span>
                        </div>
                      </td>
                    </tr>
                  );
